...
void cx(vector<int> &ch, int p1, int p2){
	vector<bool> b;
	b.resize(SIZE, false);
	ch.resize(SIZE);
	...
	a = pop[p1][0];
	x=0;
	// b[] is recording which all are in cycle
	fr(i,SIZE+1){
		if(b[a]){
			break;
		}	
...
void erx(vector<int> &ch, int p1, int p2){
	ch.resize(SIZE);
	vector<bool> b;
	b.resize(SIZE, false);
	...
	fr(i,SIZE){
		a = pop[p1][i];
		x = pop[p1][(i+1)%SIZE];
		if(i==0) y = pop[p1][SIZE-1];
		else y = pop[p1][i-1];

		w = pop[p2][(i+1)%SIZE];
		if(i==0) z = pop[p2][SIZE-1];
		else z = pop[p2][i-1];

		mp[a].push_back(x);
		if(y!=x) mp[a].push_back(y);
		if(w!=y && w!=x) mp[a].push_back(w);
		if(z!=y && z!=x && z!=w) mp[a].push_back(z);
	}
	int l = 0;
	x = pop[p1][0];
	...
	while(l<=SIZE){
		...
		b[x] = true;
		if(l>=SIZE) break;
		// remove x from the neighbour lists
		fr(i,SIZE){
			if(i==x) continue;
			bool bx = false;
			int jj = 0;
